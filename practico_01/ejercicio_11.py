...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    """
    return (sum([(elem ** 3) for elem in numeros if (elem ** 3) % 2 == 0]))
...
numeros_ordenada = sorted(numeros, key=lambda x: (x % 2 == 0)) #Los pares iran al final
